...
const Illustrations = () => {
    ...
    const fetchIllustrations = async () => {
        try {
            setLoading(true);
            const response = await axios.get('http://localhost/bfiro_backend/fetch/site/products/getProducts.php?type=Illustrations');
            ...
            const illustrations = response.data.data || [];

            const transformedData = illustrations.map(item => ({
                id: item.id,
                date: item.releaseDate ? item.releaseDate.split(' ')[0] : 'N/A',
                name: item.title,
                format: item.formats ? item.formats.map(f => f.text).join(', ') : 'SVG, AI, PNG',
            ...
            alert('Failed to load illustrations. Please try again.');  // Replace with your UI lib
            setData([]);
