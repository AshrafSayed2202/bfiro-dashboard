...
const EditUIKits = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            alert('UI Kit updated successfully!');
            setIsEditMode(false);
            // Optionally refetch data
            window.location.reload();
        } catch (error) {
            console.error('Error updating product:', error);
            alert('Failed to update UI Kit. Please try again.');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async () => {
        if (deleting) return;
        if (!window.confirm('Are you sure you want to delete this UI Kit? This action cannot be undone.')) return;

        setDeleting(true);

        try {
            await axios.post(`http://localhost/bfiro_backend/actions/products/deleteProduct.php?id=${id}`);
            alert('UI Kit deleted successfully!');
            navigate('/ui-kits');
        } catch (error) {
            console.error('Error deleting product:', error);
            alert('Failed to delete UI Kit. Please try again.');
        } finally {
            setDeleting(false);
        }
    };
    ...
    return (
        <div>
            <Header title={isEditMode ? "Edit UI Kit" : "UI Kit Details"} />
            ...
            <div className="mt-8 bg-[#171718CC] p-8 rounded-[20px] max-w-7xl mx-auto">
                ...
                {!isEditMode && (
                    <div className="flex justify-end mb-8 gap-4">
                        ...
                        >
                            {deleting ? 'Deleting...' : 'Delete UI Kit'}
                        </button>
                    </div>
                )}
