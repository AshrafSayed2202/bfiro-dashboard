...
const CampDetails = () => {
  ...
  const openUserModal = (user = null) => {
    if (user) {
      ...
      setCurrentUser({
        ...
        certificate: null,
        certificatePreview: user.certificate
          ? baseCertificateUrl + user.certificate
          : null,
      });
    } else {
      setIsUserEdit(false);
  ...
  const handleDeleteUser = async (userId) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this user from the camp?"
      )
    )
      return;
  ...
  const userColDefs = [
    ...
    { field: "enrolled_at", headerName: "Enrolled At", flex: 1 },
    {
      ...
      cellRenderer: (params) =>
        params.value ? (
          <a
            href={baseCertificateUrl + params.value}
            target="_blank"
    ...
    <div>
              ...
              <>
                ...
                <div className="mb-4">
                  ...
                  {currentUser.certificatePreview && (
                    <a
                      href={currentUser.certificatePreview}
                      target="_blank"
                      rel="noopener noreferrer"
      ...
      {showSessionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[#171718CC] p-8 rounded-[20px] w-96">
            <h2 className="text-white text-2xl mb-4">
              {isSessionEdit ? "Edit Lesson" : "Add Lesson"}
            </h2>
            {error && <p className="text-red-500 mb-4">{error}</p>}
            <div className="mb-4">
              ...
              <input
                ...
                value={
                  currentSession.starts_at
                    ? currentSession.starts_at.slice(0, 16)
                    : ""
                }
