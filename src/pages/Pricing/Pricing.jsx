...
const Pricing = () => {
    ...
    const fetchPricing = async () => {
        try {
            setLoading(true);
            const response = await axios.get('http://localhost/bfiro_backend/fetch/admin/pricing/pricing.php');
            ...
            const design = pricingData.find(item => item.title === 'Design') || { price: 0, discount: 0 };
            const subscription = pricingData.find(item => item.title === 'Subscription') || { price: 0, discount: 0 };
            const yearlyAccess = pricingData.find(item => item.id == 2) || { price: 0, discount: 0 };
            const uxCamp = pricingData.find(item => item.id == 3) || { price: 0, discount: 0 };

            setDesignPrice(parseFloat(design.price) || 0);
            setDesignOldPrice((parseFloat(design.price) || 0) + (parseFloat(design.discount) || 0));

            setSubscriptionPrice(parseFloat(subscription.price) || 0);
            setSubscriptionOldPrice((parseFloat(subscription.price) || 0) + (parseFloat(subscription.discount) || 0));

            setYearlyAccessPrice(parseFloat(yearlyAccess.price) || 0);
            setYearlyAccessOldPrice((parseFloat(yearlyAccess.price) || 0) + (parseFloat(yearlyAccess.discount) || 0));

            setUxCampPrice(parseFloat(uxCamp.price) || 0);
            setUxCampOldPrice((parseFloat(uxCamp.price) || 0) + (parseFloat(uxCamp.discount) || 0));
        } catch (error) {
    ...
    const handleSave = async () => {
        try {
            const response = await axios.post('http://localhost/bfiro_backend/actions/admin/pricing/edit.php', {
                designPrice,
                designOldPrice,
                subscriptionPrice,
                ...
                <div className="bg-[#171718CC] p-5 rounded-[20px] flex-1 min-w-[200px]">
                    ...
                    <div className="mt-4">
                        ...
                        <input
                            ...
                            onChange={(e) => setDesignOldPrice(parseFloat(e.target.value) || 0)}
                            className="mt-1 bg-gray-800 text-white p-2 rounded w-full border border-gray-700 focus:outline-none focus:border-blue-500"
                        />
                    </div>
                </div>
                <div className="bg-[#171718CC] p-5 rounded-[20px] flex-1 min-w-[200px]">
                    ...
                    <div className="mt-4">
                        ...
                        <input
                            ...
                            onChange={(e) => setSubscriptionOldPrice(parseFloat(e.target.value) || 0)}
                            className="mt-1 bg-gray-800 text-white p-2 rounded w-full border border-gray-700 focus:outline-none focus:border-blue-500"
                        />
                    </div>
                </div>
                <div className="bg-[#171718CC] p-5 rounded-[20px] flex-1 min-w-[200px]">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => setYearlyAccessPrice(parseFloat(e.target.value) || 0)}
                            className="mt-1 bg-gray-800 text-white p-2 rounded w-full border border-gray-700 focus:outline-none focus:border-blue-500"
                        />
                    </div>
                    <div className="mt-4">
                        ...
                        <input
                            ...
                            onChange={(e) => setYearlyAccessOldPrice(parseFloat(e.target.value) || 0)}
                            className="mt-1 bg-gray-800 text-white p-2 rounded w-full border border-gray-700 focus:outline-none focus:border-blue-500"
                        />
                    </div>
                </div>
                <div className="bg-[#171718CC] p-5 rounded-[20px] flex-1 min-w-[200px]">
                    ...
                    <div className="mt-4">
                        ...
                        <input
                            ...
                            onChange={(e) => setUxCampOldPrice(parseFloat(e.target.value) || 0)}
                            className="mt-1 bg-gray-800 text-white p-2 rounded w-full border border-gray-700 focus:outline-none focus:border-blue-500"
                        />
                    </div>
                </div>
